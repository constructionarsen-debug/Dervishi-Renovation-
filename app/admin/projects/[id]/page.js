    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-sm text-gray-600 dark:text-gray-300">Këtu mund të shtosh ose fshish foto/video individuale.</div>
        </div>
      ...
      <form action="/api/admin/projects" method="POST" className="rounded-[2rem] border border-black/10 bg-white p-7 shadow-sm dark:border-white/10 dark:bg-gray-900">
        ...
        <input id="project_images_edit" name="images" type="hidden" defaultValue={JSON.stringify(project.images || [])} />
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            <HiddenSingleMediaManager inputId="project_coverImage_edit" title="Cover aktual" emptyText="Nuk ka cover ende." />
            <HiddenMediaArrayManager inputId="project_images_edit" title="Foto / video aktuale" emptyText="Nuk ka media në galeri ende." />
          </div>
        </div>
      </form>
    </div>
